...
int select(const std::span<int> population,
           const std::span<const int> fitness_values, int total_fitness,
  ...
  case SelectionMethod::TOURNAMENT:
    throw std::logic_error(
        "Metoda selekcji TOURNAMENT nie jest jeszcze zaimplementowana.");
  case SelectionMethod::RANK:
    throw std::logic_error(
        "Metoda selekcji RANK nie jest jeszcze zaimplementowana.");
  }
}
